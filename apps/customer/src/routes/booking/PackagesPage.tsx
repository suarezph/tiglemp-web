...
export function PackagesPage() {
  ...
  const partner = findStubPartner(partnerId);
  const packages = useMemo(
    () =>
      getStubPackagesForPartner(partnerId, draft.search.serviceTypeCode),
    [partnerId, draft.search.serviceTypeCode]
  );

  const customOnly = packages.length === 0;
    ...
    <div className="space-y-6">
      <header className="rounded-2xl bg-white ring-1 ring-border p-5 md:p-6">
        <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
          {draft.partner?.autoAssigned ? 'Auto-assigned business' : 'Selected business'}
        </p>
                  ...
                  <div className="flex flex-wrap items-start gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="font-bold text-lg">
                        ₱{pkg.priceFromPHP.toLocaleString()}
                      </p>
